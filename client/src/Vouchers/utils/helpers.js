...
// ── Currency conversion ───────────────────────────────────────────────────────
// exchange_rate is stored as: how many BASE units = 1 of this currency.
// So:  baseAmount   = amount * fromRate
//      targetAmount = baseAmount / toRate
export function findBaseCurrency(currencies = []) {
  return currencies.find((c) => c.is_base) || null;
}

export function rateOf(currency) {
  return parseFloat(currency?.exchange_rate ?? 1) || 1;
}

export function convertAmount(amount, fromCurrency, toCurrency) {
  const amt = parseFloat(amount);
  if (!isFinite(amt) || !fromCurrency || !toCurrency) return 0;
  const base = amt * rateOf(fromCurrency);   // → base currency
  return base / rateOf(toCurrency);          // → target currency
}

// Effective rate to convert 1 unit of `from` into `to`.
export function pairRate(fromCurrency, toCurrency) {
  if (!fromCurrency || !toCurrency) return 0;
  return rateOf(fromCurrency) / rateOf(toCurrency);
}
