...
export function Skeleton({ width, height, style = {} }) {
  return <div className="skeleton" style={{ width: width || '100%', height: height || '16px', ...style }} />;
}
...
export function TableSkeleton({ rows = 6, cols = 6 }) {
  return (
    <table className="data-table" style={{ width: '100%' }}>
      <thead>
        <tr>{Array.from({ length: cols }).map((_, i) => <th key={i}><Skeleton width="80%" /></th>)}</tr>
      </thead>
      <tbody>
        ...
        <div className="error-alert" style={{ margin: '16px 0' }}>
          ...
          <div>
            ...
            <div style={{ marginTop: '8px' }}>
              <button className="btn btn-secondary btn-sm" onClick={() => this.setState({ hasError: false, error: null })}>
                Retry
              </button>
            </div>
          </div>
    ...
    <div className="modal-overlay" onClick={handleOverlayClick} role="dialog" aria-modal="true">
      <div className={`modal modal-${size}`}>
        <div className="modal-header">
          <h3 style={{ fontSize: '1.1em', fontWeight: 600 }}>{title}</h3>
...
export function ConfirmDialog({ open, onClose, onConfirm, title, message, confirmLabel = 'Confirm', danger = false, loading = false }) {
  return (
    <Modal
      open={open} onClose={onClose} title={title || 'Confirm Action'} size="sm"
      footer={
        <>
    ...
    >
      <p style={{ color: 'var(--text-secondary)', fontSize: '0.95em', lineHeight: 1.6 }}>{message}</p>
    </Modal>
  );
}
...
export function Pagination({ page, totalPages, onPageChange }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', justifyContent: 'center', padding: '14px' }}>
      <button className="btn btn-secondary btn-sm" onClick={() => onPageChange(page - 1)} disabled={page <= 1}>← Prev</button>
      <span style={{ fontSize: '0.85em', color: 'var(--text-muted)', padding: '0 8px' }}>Page {page} of {totalPages}</span>
