...
class VoucherDetailInline(admin.TabularInline):
    model       = VoucherDetail
    extra       = 2
    fields      = ['account_id', 'description', 'debit', 'credit', 'cost_center']
    show_change_link = False

    def get_readonly_fields(self, request, obj=None):
        # Lock all fields once a voucher is posted
...
class VoucherHeaderAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Voucher Info', {
            'fields': ('voucher_no', 'v_type', 'date', 'reference',
                       'narration', 'status')
        }),
        ('Currency', {
...
class CurrencyAdmin(admin.ModelAdmin):
    list_display  = ['code', 'name', 'symbol',
                     'exchange_rate', 'is_base', 'is_active', 'updated_at']
...
class RecurringScheduleAdmin(admin.ModelAdmin):
    list_display  = ['template_voucher', 'frequency',
                     'next_due_date', 'times_generated', 'is_active']
