...
import { useVouchers, useToast } from '../../hooks/useFinTrack';
...
export default function VoucherList() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterType)   data = data.filter((v) => v.voucher_type === filterType);
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      toast.success(`${confirmPost.voucher_number} posted successfully.`, 'Posted');
      refetch();
    } catch (err) {
  ...
  return (
    <ErrorBoundary>
      ...
      <div className="voucher-toolbar">
        ...
        <button className="btn btn-primary" onClick={() => { setEditVoucher(null); setShowForm(true); }}>
          + New Voucher
        </button>
      </div>
            ...
            <thead>
              <tr>
                <th className="sortable" onClick={() => handleSort('voucher_number')}>
                  Voucher No.<SortIcon field="voucher_number" />
                </th>
                <th>Type</th>
                <th className="sortable" onClick={() => handleSort('voucher_date')}>
                  Date<SortIcon field="voucher_date" />
                </th>
                <th>Description</th>
                <th>Status</th>
                <th className="sortable amount-cell" onClick={() => handleSort('total_amount')}>
              ...
              {paginated.map((v) => {
                ...
                return (
                  <tr key={v.id}>
                    ...
                    <td style={{ whiteSpace: 'nowrap' }}>{formatDate(v.voucher_date)}</td>
                    <td className="desc-cell">{v.description || '—'}</td>
                    <td><span className={`badge ${statusConf.color || ''}`}>{statusConf.label || v.status}</span></td>
                    <td className="amount-cell">{formatCurrency(v.total_amount, v.currency_code)}</td>
                    <td>
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to delete voucher ${confirmDelete?.voucher_number}? This action cannot be undone.`}
        confirmLabel="Delete"
      ...
      <ConfirmDialog
        ...
        title="Post Voucher"
        message={`Post ${confirmPost?.voucher_number}? Once posted, the voucher becomes immutable and cannot be edited or deleted.`}
